...
dnn.append(ops.Concat([dataset.images, dnn[-1]],axis=0))
...
def compute_row(i):
    ...
    return tf.reduce_sum(tf.square(grad[:32]-grad[32:]), [1, 2, 3])
#tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.square(grad[:32]-grad[32:]),
#                          [1, 2, 3])+0.001))

prediction = dnn[-1]
hessian = tf.reduce_mean(tf.sqrt(tf.reduce_sum(tf.map_fn(compute_row,
                       tf.range(dataset.n_classes), dtype=tf.float32), 0) + 0.0001))

accu = sknet.losses.streaming_mean(sknet.losses.accuracy(dataset.labels,
                                                         dnn[-1][:32]))
loss = sknet.losses.crossentropy_logits(dataset.labels, dnn[-1][:32])

# optimizer and updates
B = dataset.N_BATCH('train_set')
lr = sknet.schedules.PiecewiseConstant(LR, {70*B: LR/3, 120*B: LR/9})
optimizer = sknet.optimizers.Adam(loss+GAMMA*hessian,
...
valid = sknet.Worker(loss=loss, accu=accu, hessian=hessian,
                     ...
                     feed_dict={**dnn[1:].deter_dict(True),
                                **dnn[:1].deter_dict(False)})
inter = sknet.Worker(inter=dnn[-1], context='interpolation',
                     name='interp', feed_dict=dnn.deter_dict(True))
...
for run in range(5):
    workplace.init_file(path.format(MODEL, DATASET, N, GAMMA, LR, run))
