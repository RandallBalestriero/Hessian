...
def plotit(NAME):
    ...
    train_tlabels = f['train_set/loss/4'][...].flatten()
    train_accu = f['train_set/loss/5'][...].flatten()
    ...
    plt.plot(accu,'--k')
    plt.plot(train_accu,'--b')

    plt.tight_layout()
    plt.savefig('images/'+NAME+'_comparison.pdf')
    print('SCORES:',TRAIN_a[-1],TEST_a[-1],train_accu[-1],accu[-1])
...
for dataset in ['mnist','cifar10']:
    for proportion in [0,0.25,0.5,0.75]:
        for model in ['cnn','resnetsmall','resnetlarge','mlpsmall','mlplarge']:
            NAME = 'random_{}_{}_{}'.format(dataset, model, proportion)
            plotit(NAME)
